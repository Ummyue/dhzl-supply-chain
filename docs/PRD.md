...
### 1.1 项目背景
大河智链物流（郑州）有限公司是大河控股冷链物流投资运营平台，定位于服务冻品（牛肉/羊肉/水产等冷链）供应链金融业务。本次系统建设旨在重构升级原中信梧桐港承建的供应链管理系统，解决流程不通、风控薄弱、系统对接不畅等问题，**一期 7 月中旬上线，二期 8 月底（适配期现货代采），三期 9 月底（B2B 交易平台）**。
...
### 3.3 核心业务数据样例
- **客户**：郑州军牧源贸易有限公司（统一社会信用代码 91XXXXXXXXMAXXXXXXXX）、郑州固联冷链有限公司
...
| **Topbar 角色标识** | `货主方 · 盖章人` + `军牧源 · 李雪`（橙色徽标，与操作人区分） |
...
| 字段 | 过滤条件 | 备注 |
|---|---|---|
| customerCode | === `MockData.users.customer_seal.company` 对应的客户编码 | 军牧源 = `JMY` |
...
| customerCode 客户简称 | string | JMY=军牧源，GL=固联 |
